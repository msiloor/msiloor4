...
export interface Params {
  imageUrl: string
  skewPosition?: 'top' | 'bottom'
  clipHeight?: string | number
  minification?: number
  cb: (rgba: string, hsla: string, r: number, g: number, b: number, a: number, h: number, s: number, l: number) => void
}
export function getImageMeanColor(params: Params) {
  /* 图片的url */
  ...
  function confirmHeight() {
    if (typeof clipHeight === 'string') {
      ...
      if (isNaN(Number(clipHeight))) {
        if (/%$/.test(clipHeight)) {
          canvasHeight = (imageHeight * Number(clipHeight.replace(/%$/, ''))) / 100
